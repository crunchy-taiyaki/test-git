...
write(6,*) 'GUESS_TEMP'
#endif
write(6,*) 'choose escape probability mode (full/lvg)'
read(*,*) mode
...
allocate(lines_escape_probability(1:7,0:pdr_ptot))
allocate(lines_tau(1:7,0:pdr_ptot))
...
!OUTPUT FOR TAU
!-------------------------------------
out_file = trim(adjustl(directory))//'/'//"tau"
out_file2 = trim(adjustl(out_file))//"]"
write(6,'(" Writing file [",A)') out_file2
open(unit=11,file=out_file,status='replace') 
    do pp=1,pdr_ptot
       write(11,*) lines_tau(:,pp)
    enddo
 close(11)
!-------------------------------------
...
DO pp=1,pdr_ptot
  ...
  star_dist = sqrt((UV_source_coord(1) - pdr(p)%x)**2 + (UV_source_coord(2) - pdr(p)%y)**2  +&
              (UV_source_coord(3) - pdr(p)%z)**2 )
  ENDIF
  ...
  DO j=0,nrays-1
     IF (pdr(p)%epray(j).GT.0) THEN 
        DO i=1,pdr(p)%epray(j)
           ...
           if ((j.eq.5).and.(fieldchoice.EQ."PNT")) then
              dist = SQRT((pdr(p)%epoint(1,j,i)-pdr(p)%epoint(1,j,0))**2 + &
                & (pdr(p)%epoint(2,j,i)-pdr(p)%epoint(2,j,0))**2 + &
                & (pdr(p)%epoint(3,j,i)-pdr(p)%epoint(3,j,0))**2)
              if (dist.gt.star_dist) exit
  ...
  DO j=0,nrays-1
    ...
    IF (fieldchoice.EQ."PNT") THEN
       if (j.eq.5) then
          pdr(p)%rad_surface(j) = Gext(1) 
       else
          pdr(p)%rad_surface(j) = 0.0D0
       endif     
    ENDIF
   ...
   DO j=0,nrays-1
            ...
            !if ((pdr(p)%AV(j).eq.0.0).and.(j.eq.5)) then               
            !write(77,*)p, pdr(p)%x, pdr(p)%y, pdr(p)%z
            !write(6,*)p, pdr(p)%x, pdr(p)%y, pdr(p)%z
            !endif
