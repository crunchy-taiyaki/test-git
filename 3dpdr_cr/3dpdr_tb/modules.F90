...
MODULE maincode_module
  ...
  real(kind=DP) :: max_x       ! Maximum x value
...
!  real(kind=dp) :: h
  ...
  !=====B. Gaches Cosmic Ray spectrum variables
  !=====START COSMIC RAYS
  integer(kind=4) :: numCRsources !Number of CR sources
  integer(kind=4) :: crNene ! All spectrum must have same number of energy bins
  character(len=3), allocatable :: crfieldchoice(:) !ISO (external) or PNT
  character(len=20), allocatable :: crInputFiles(:) !Files providing the spectrum as (E, F(E))
  real(kind=DP), allocatable :: crSpectrum(:, :) !CR flux spectrum
  real(kind=DP), allocatable :: crEnergy(:,:) !Energys for the CR flux
  real(kind=DP), allocatable :: crRadialScaling(:) !for flux scaling for PNT sources
	integer(kind=4), allocatable :: crDirection(:)
	!Dummy variables for temporarily storing attenauted spectrum
	real(kind=dp), allocatable :: crEnergy_A(:)
	real(kind=dp), allocatable :: crSpectrum_A(:)
	real(kind=DP) :: zetaDum, crindex
  !=====END COSMIC RAYS
...
!====COSMICRAYS - START =====
     real(kind=dp) :: zetacr   !cosmic ray for pdr point
	 !Store the flux in each cell too
	 real(kind=dp), allocatable :: crEnergy(:)
	 real(kind=dp), allocatable :: crSpectrum(:)
!====COSMICRAYS - END =====
