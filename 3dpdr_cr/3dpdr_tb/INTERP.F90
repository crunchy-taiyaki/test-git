...
!
!    Output, real ( kind = DP ) X(N), the abscissas.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = DP ) A, B, the endpoints of the interval.
!
!    Input, integer ( kind = 4 ) N, the order of the rule.
!
!    Output, real ( kind = DP ) X(N), the abscissas.
...
!
  ...
  integer ( kind = 4 ) i
  real ( kind = DP ) :: pi = 3.141592653589793D+00
  real ( kind = DP ) theta(n)
  real ( kind = DP ) x(n)
  ...
  do i = 1, n
    theta(i) = real ( 2 * n - 2 * i + 1, kind = DP ) * pi &
             / real ( 2 * n,             kind = DP )
  end do
...
!
!    Input, real ( kind = DP ) T_DATA(DATA_NUM), the value of the
!    independent variable at the sample points.
!
!    Input, real ( kind = DP ) P_DATA(M,DATA_NUM), the value of the
...
!
!    Input, real ( kind = DP ) T_INTERP(INTERP_NUM), the value of the
!    independent variable at the interpolation points.
!
!    Output, real ( kind = DP ) P_INTERP(M,DATA_NUM), the interpolated
...
!
  ...
  real ( kind = DP ) l_interp(data_num,interp_num)
  real ( kind = DP ) p_data(m,data_num)
  real ( kind = DP ) p_interp(m,interp_num)
  real ( kind = DP ) t_data(data_num)
  real ( kind = DP ) t_interp(interp_num)
!
...
!
!    Input, real ( kind = DP ) T_DATA(DATA_NUM), the data points.
!
!    Input, integer ( kind = 4 ) INTERP_NUM, the number of
!    interpolation points.
!
!    Input, real ( kind = DP ) T_INTERP(INTERP_NUM), the
!    interpolation points.
!
!    Output, real ( kind = DP ) L_INTERP(DATA_NUM,INTERP_NUM), the values
...
!
  ...
  do i = 1, n
    x(i) = ( real ( n - i + 1, kind = DP ) * ( -1.0D+00 )   &
           + real (     i,     kind = DP ) * ( +1.0D+00 ) ) &
           / real ( n     + 1, kind = DP )
  end do
...
!    Input, integer ( kind = 4 ) DATA_NUM, the number of data points.
!
!    Input, real ( kind = DP ) P_DATA(M,DATA_NUM), the data values.
!
!    Output, real ( kind = DP ) T_DATA(DATA_NUM), parameter values
...
!
!    Input, real ( kind = DP ) A(M,N), a "small" M by N array.
!
!    Input, integer ( kind = 4 ) M2, N2, the number of rows and columns in A2.
!
!    Output, real ( kind = DP ) A2(M2,N2), the expanded array, which
...
!
  ...
  do i = 1, m2
    ...
    r1 = real ( i1 - 1, kind = DP ) &
       / real ( m - 1, kind = DP )

    r2 = real ( i2 - 1, kind = DP ) &
       / real ( m - 1, kind = DP )
    ...
    do j = 1, n2
      ...
      s1 = real ( j1 - 1, kind = DP ) &
         / real ( n - 1, kind = DP )

      s2 = real ( j2 - 1, kind = DP ) &
         / real ( n - 1, kind = DP )
...
!
!    Input, real ( kind = DP ) X(N), the array to be examined.
!
...
!
!    Input, real ( kind = DP ) X(N), an array sorted into ascending order.
!
...
!
  ...
  do i = 1, n - 1
    ...
    do j = 1, fat
      k = k + 1
      xfat(k) = ( real ( fat - j + 1, kind = DP ) * x(i)     &
                + real (       j,     kind = DP ) * x(i+1) ) &
                / real ( fat     + 1, kind = DP )
    end do
...
!
!    Output, real ( kind = DP ) XFAT(BEFORE+(N-1)*(FAT+1)+1+AFTER), the
!    "fattened" data.
!
  implicit none
  ...
  integer ( kind = 4 ) k
  real ( kind = DP ) x(n)
  real ( kind = DP ) xfat(before+(n-1)*(fat+1)+1+after)
...
!
  do j = 1 - before + fat, fat
    k = k + 1
    xfat(k) = ( real ( fat - j + 1, kind = DP ) * ( x(1) - ( x(2) - x(1) ) ) &
              + real (       j,     kind = DP ) *   x(1)          ) &
              / real ( fat     + 1, kind = DP )
  end do
!
...
!
  do j = 1, after
    k = k + 1
    xfat(k) = &
      ( real ( fat - j + 1, kind = DP ) * x(n)     &
      + real (       j,     kind = DP ) * ( x(n) + ( x(n) - x(n-1) ) ) ) &
      / real ( fat     + 1, kind = DP )
  end do
...
!
!    Input, real ( kind = DP ) A(N), a sorted vector.
!
!    Input, real ( kind = DP ) VALUE, the value whose nearest vector
!    entry is sought.
!
