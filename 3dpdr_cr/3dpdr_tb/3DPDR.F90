...
real(kind=dp)::uvfieldaux, coldum, crtest
...
close(12)
write(6,*) '============================================='
!===B. Gaches - Write CR Stuff out
write(6,*) "Number of CR Sources: 				", numCRsources
write(6,*) "===================================="
do i = 1, numCRsources
	write(6,*) "Field Choice: ", crfieldchoice(i)
	write(6,*) "File Input: ", crInputFiles(i)
	write(6,*) "Radial Fac: ", crRadialScaling(i)
	write(6,*) "===================================="
end do
write(6,*) ''
write(6,*) '------FLAGS-----'
...
do p=1,grand_ptot
	...
	!BALG - Hack to make zeta read in
	read(2,*) xpos,ypos,zpos,denst
...
!==========
	...
	!START COSMIC Ray
	allocate(pdr(p)%crEnergy(crNene))
	allocate(pdr(p)%crSpectrum(crNene))
	!END COSMIC RAY
!==========
enddo
...
!This is where you need to call your routine.
!The column density of a particle (pdr) is: pdr(p)%columndensity(j),
!where j is the healpix ray. For 1D, you will consider j=6 only.
!For all other rays you can assume an extremely large column density (e.g. infinite)
!do pp=1,pdr_ptot
!  p=IDlist_pdr(pp)
!  pdr(p)%zetacr=1d-15/1.3d-17 !<<---- in principle this is where you will call your routine
                              !input: pdr(p)%columndensity(6), output: zetacr value.
!DONT FORGET TO NORMALIZE IT WITH 1.3d-17 AS SHOWN IN THE ABOVE EXAMPLE!!!
!In the above example, I assing to all particles a zeta_cr = 1e-15 s-1.
!Also, make sure you use 1d-xx, not 1e-xx, as it is double precision. If you use 1e-xx it won't work.
!enddo
write(6, *) "Setting up Cosmic Ray Attenuation Solver"
call CRsetup()
write(6, *) "Calculating Cosmic Ray Ionization Rates"
write(6, *) "Radius = ", max_x
do pp=1, pdr_ptot
	zetaDum = 0.0
	p = IDlist_pdr(pp)
	pdr(p)%zetacr = 0.0
	pdr(p)%crSpectrum = 0d0
	!Do 1D case, no need to loop over rays then
	coldum = 1.0d25
	do i = 1, numCRsources
		if(crDirection(i).EQ.1) then
			!coldum = column(pp)%columndens_point(6, NH2)
			coldum=pdr(p)%columndensity(6)
		endif
		if(crDirection(i).EQ.-1) then
			!coldum = column(pp)%columndens_point(4, NH2)
			coldum = pdr(p)%columndensity(4)
		endif
		call spec_atten(crEnergy(i, :), crSpectrum(i, :), crSpectrum_A, crEnergy_A, coldum)
		if (crfieldchoice(i).EQ."SRC") then
			if(crDirection(i).EQ.-1) then
				crtest = (max_x - MIN(pdr(p)%x, max_x))
				if(crtest.lt.0.0) write(6, *) "FATAL ERROR IN CR: dr  < 0", crtest, radius, pdr(p)%x
				crSpectrum_A = crSpectrum_A * (crRadialScaling(i)/((max_x - MIN(pdr(p)%x, max_x)) + crRadialScaling(i)))**crindex
			else if(crDirection(i).EQ.1) then
				crSpectrum_A = crSpectrum_A * (crRadialScaling(i)/(pdr(p)%x + crRadialScaling(i)))**crindex
			endif
		endif
		pdr(p)%crEnergy = crEnergy_A
		pdr(p)%crSpectrum = pdr(p)%crSpectrum + crSpectrum_A
		call ionRate(crEnergy_A, crSpectrum_A, zetaDum)
		!pdr(p)%crEnergy(:) = crEnergy(i,:)
		!pdr(p)%crSpectrum(:) = pdr(p)%crSpectrum(:) + crSpectrum(i,:)
		!call ionRate(crEnergy(i,:), crSpectrum(i,:), zetaDum)
		if(crfieldchoice(i).EQ."ISO") zetaDum = zetaDum * 2.0
		pdr(p)%zetacr = pdr(p)%zetacr + ((5.0/3.0)*zetaDum)/1.3d-17 !2ndaries
	end do
end do
open(93, file="zeta.txt", status="replace")
do pp=1, pdr_ptot
	p = IDlist_pdr(pp)
	write(93, *) pdr(p)%x, pdr(p)%zetacr, column(pp)%columndens_point(6, NH2)
end do
!-----------------------------------------
!START OUTPUT FOR COSMIC RAY SPECTRUM
!-----------------------------------------
out_file = trim(adjustl(output))//".CR.tmp"
...
do pp=1, pdr_ptot
	p=IDlist_pdr(pp)
	if(pp == 1) write(13,'(I7, 50ES12.5)') crNene, pdr(p)%crEnergy
	write(13,'(I7, 50ES12.5)') p, pdr(p)%x, pdr(p)%crSpectrum
end do
close(13)
#endif
!-----------------------------------------
!END OUTPUT FOR COSMIC RAY SPECTRUM
!-----------------------------------------
close(93)
!===== COSMICRAYS - END ========
...
!===== COSMICRAYS - START =====
    CALL CALCULATE_REACTION_RATES(gastemperature(pp),dusttemperature(pp),nrays,pdr(p)%rad_surface(0:nrays-1),&
          ...
          &NRGR,NRH2,NRHD,NRCO,NRCI,NRSI,pdr(p)%zetacr)
!===== COSMICRAYS - END =====
      DUMMY_RATE(:,pp) = rate
...
!$OMP END PARALLEL DO
   ...
   call calc_columndens
	 !=== PUT IN NEW CALCULATION OF COSMIC RAYS HERE AFTER EACH CHEMICAL ITERATION ===
	write(6, *) "Calculating Cosmic Ray Ionization Rates"
	do pp=1, pdr_ptot
		zetaDum = 0.0
		p = IDlist_pdr(pp)
		pdr(p)%zetacr = 0.0
		pdr(p)%crSpectrum = 0.0
		!Do 1D case, no need to loop over rays then
		coldum = 1.0d25
		do i = 1, numCRsources
			if(crDirection(i).EQ.1) then
			...
			!call ionRate(crEnergy(i,:), crSpectrum(i,:), zetaDum)
      if(crfieldchoice(i).EQ."ISO") zetaDum = zetaDum * 2.0
			pdr(p)%zetacr = pdr(p)%zetacr + ((5.0/3.0)*zetaDum)/1.3d-17
!factor of 1.4 for secondaries not being treated YET
		end do
	end do
	open(93, file="zeta.txt", status="replace")
	do pp=1, pdr_ptot
...
!---------------------------
   ...
   do pp=1,pdr_ptot
      p=IDlist_pdr(pp)
#ifdef PSEUDO_1D
      write(13,'(I7,200ES11.3E3)') p, pdr(p)%x, pdr(p)%AV(6), CII_cool(pp),CI_cool(pp), &
                               &OI_cool(pp),C12O_cool(pp), total_cooling_rate(pp)
#else
      write(13,'(I7,200ES11.3E3)') p, pdr(p)%x, pdr(p)%y, pdr(p)%z, CII_cool(pp), CI_cool(pp),&
...
!---------------------------
   ...
   do pp=1,pdr_ptot
      p=IDlist_pdr(pp)
#ifdef PSEUDO_1D
      write(14,'(I7,200ES12.3E3)') p, pdr(p)%x, pdr(p)%AV(6), all_heating(pp,:)
#else
      write(14,'(I7,200ES12.3E3)') p, pdr(p)%x, pdr(p)%y, pdr(p)%z, all_heating(pp,:), pdr(p)%AV(:)
#endif
   enddo
   close(14)
!-------------------------------
...
#else
      write(16,'(I9,200ES11.3)') pp, pdr(p)%x, pdr(p)%y, pdr(p)%z, pdr(p)%CII_pop, pdr(p)%CI_pop, pdr(p)%OI_pop, pdr(p)%C12O_pop
#endif
   enddo
   close(16)
!--------------------------------
...
!---------------------------
   ...
   do pp=1,pdr_ptot
      p=IDlist_pdr(pp)
      write(16,'(I9,200ES11.3)') pp, pdr(p)%AV(6),&     !ID,AV
...
OPEN(unit=93, file="testFUV.dat", status='replace')
DO pp=1,pdr_ptot
  p=IDlist_pdr(pp)
  write(93,*) pdr(p)%x, pdr(p)%UVfield
 END DO
 CLOSE(93)
