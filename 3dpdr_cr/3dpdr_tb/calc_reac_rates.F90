...
!======= COSMICRAYS - START =====
      SUBROUTINE CALCULATE_REACTION_RATES(TEMPERATURE,DUST_TEMPERATURE,NRAYS,RAD_SURFACE, &
               ...
               & NRGR,NRH2,NRHD,NRCO,NRCI,NRSI,zetacrlocal)
!======= COSMICRAYS - END =====
 ...
 use maincode_module , only : AV_fac
...
!======= COSMICRAYS - START =====
      real(kind=dp),intent(in)::zetacrlocal
!======= COSMICRAYS - END =====
...
!======= COSMICRAYS - START =====
            RATE(I)=ALPHA(I)*ZETAcrlocal*(TEMPERATURE/300.0D0)**BETA(I)&
     &           *GAMMA(I)/(1.0D0-OMEGA)
!======= COSMICRAYS - END =====
         ELSE IF(DUPLICATE(I).EQ.1) THEN
            J=I
            DO
               IF(TEMPERATURE.LE.RTMAX(J)) THEN
!======= COSMICRAYS - START =====
                  RATE(J)=ALPHA(J)*ZETAcrlocal*(TEMPERATURE/300.0D0)**BETA(J)&
     &                 *GAMMA(J)/(1.0D0-OMEGA)
!======= COSMICRAYS - END =====
                  EXIT
               ELSE IF(DUPLICATE(J+1).LT.DUPLICATE(J)) THEN
...
!======= COSMICRAYS - START =====
         RATE(I)=FLUX*ZETAcrlocal*2.4D-22*YIELD
!======= COSMICRAYS - END =====
         GOTO 10
