...
!
!    Input, real ( kind = DP ) XTAB(NTAB), the abscissas at which the
!    function values are given.  The XTAB's must be distinct
!    and in ascending order.
!
!    Input, real ( kind = DP ) YTAB(NTAB), the function values,
!    YTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = DP ) A, the lower limit of integration.  A should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Input, real ( kind = DP ) B, the upper limit of integration.  B should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
	use definitions
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = DP ), external FUNC, the name of the function to be
...
!    in FUNC.
!
!    Input, real ( kind = DP ) A, the lower limit of integration.
!
!    Input, real ( kind = DP ) B, the upper limit of integration.
!
!    Input, real ( kind = DP ) ABSERR, the absolute error tolerance.
!
!    Input, real ( kind = DP ) RELERR, the relative error tolerance.
!
!    Output, real ( kind = DP ) ERROR, an estimate of the absolute error.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
...
!
  ...
  real ( kind = DP ) a
  real ( kind = DP ) abserr
  real ( kind = DP ) ait(maxtbl)
  logical aitken
  real ( kind = DP ), parameter :: aitlow = 1.1D+00
  real ( kind = DP ), parameter :: aittol = 0.1D+00
  real ( kind = DP ) astep
  real ( kind = DP ) b
  real ( kind = DP ) beg
  real ( kind = DP ) begin(mxstge)
  real ( kind = DP ) bma
  real ( kind = DP ) curest
  real ( kind = DP ) dif(maxtbl)
  real ( kind = DP ) diff
  real ( kind = DP ) end
  real ( kind = DP ) ergoal
  real ( kind = DP ) erra
  ...
  real ( kind = DP ) error
  real ( kind = DP ) errr
  real ( kind = DP ) est(mxstge)
  real ( kind = DP ) fbeg
  real ( kind = DP ) fbeg2
  real ( kind = DP ) fend
  real ( kind = DP ) fextm1
  real ( kind = DP ) fextrp
  real ( kind = DP ) finis(mxstge)
  real ( kind = DP ) fn
  real ( kind = DP ) fnsize
  ...
  real ( kind = DP ) h2next
  real ( kind = DP ) h2tfex
  real ( kind = DP ), parameter :: h2tol = 0.15D+00
  real ( kind = DP ) hovn
  integer ( kind = 4 ) i
  integer ( kind = 4 ) ibeg
  ...
  real ( kind = DP ) sum1
  real ( kind = DP ) sumabs
  real ( kind = DP ) t(maxtbl,maxtbl)
  real ( kind = DP ) tabs
  real ( kind = DP ) tabtlm
  real ( kind = DP ), parameter :: tljump = 0.01D+00
  real ( kind = DP ) ts(2049)
  real ( kind = DP ) vint
...
!    Input, real ( kind = DP ) B, the upper limit of integration.
!
!    Input, real ( kind = DP ) EPSIN, the relative error tolerance.
!
!    Output, real ( kind = DP ) EPSOUT, estimated integration error.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
  implicit none
  ...
  real ( kind = DP ) a1
  real ( kind = DP ) a2
  real ( kind = DP ) acof(257)
  real ( kind = DP ) alf
  real ( kind = DP ) alfnj
  real ( kind = DP ) alfno
  real ( kind = DP ) b
  real ( kind = DP ) bcof(257)
  real ( kind = DP ) bet
  real ( kind = DP ) betnj
  real ( kind = DP ) betno
  real ( kind = DP ) bounds
  real ( kind = DP ) ccof(513)
  real ( kind = DP ) cof
  real ( kind = DP ) cofmax
  real ( kind = DP ) const1
  real ( kind = DP ) const2
...
!  Parameters:
!
!    Input, real ( kind = DP ) ALPHA, BETA, parameters needed for Laguerre 
!    and Jacobi polynomials.
!
!    Input, integer ( kind = 4 ) KIND, specifies which polynomial is to be 
!    handled:
...
!    calculate.
!
!    Input, real ( kind = DP ) ALPHA, the value of the ALPHA parameter,
!    required only for Jacobi or Laguerre polynomials.
!
!    Input, real ( kind = DP ) BETA, the value of the BETA parameter,
!    required only for Jacobi polynomials.
!
!    Output, real ( kind = DP ) B(N-1), the offdiagonal coefficients.
!
!    Output, real ( kind = DP ) A(N), the diagonal coefficients.
!
!    Output, real ( kind = DP ) MUZERO, the zero-th moment, Integral W(X) DX,
...
!
  if ( kind == 1 ) then
    ...
    do i = 1, n-1
      b(i) = real ( i, kind = DP ) &
        / sqrt ( 4.0D+00 * real ( i * i, kind = DP ) - 1.0D+00 )
    end do
!
!  KIND = 2:
!
...
!
    ...
    a(1:n) = 0.0D+00
    do i = 1, n-1
      b(i) = sqrt ( real ( i, kind = DP ) / 2.0D+00 )
    end do
!
!  KIND = 5:
!
...
!
    ...
    do i = 1, n
      a(i) = ( beta**2 - alpha**2 ) / &
        ( ( 2.0D+00 * real ( i - 1, kind = DP ) + alpha + beta ) &
        * ( 2.0D+00 * real ( i, kind = DP ) + alpha + beta ) )
    end do
    ...
    do i = 2, n-1
      ...
      b(i) = sqrt ( 4.0D+00 * real ( i, kind = DP ) &
...
!
    ...
    do i = 1, n
      a(i) = 2.0D+00 * real ( i, kind = DP ) - 1.0D+00 + alpha
    end do
...
!
!    Input, real ( kind = DP ) XTAB(NTAB), contains the points at which the
!    function was evaluated.  The XTAB's must be distinct and
!    in ascending order.
!
!    Input, real ( kind = DP ) FTAB(NTAB), contains the tabulated values of
!    the function, FTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = DP ) A, lower limit of integration.
!
!    Input, real ( kind = DP ) B, upper limit of integration.
!
!    Output, real ( kind = DP ) Y(3,NTAB), will contain the coefficients
...
!                   + Y(3,I)*(X-XTAB(I))**3
!
!    Output, real ( kind = DP ) E(NTAB), E(I) = the definite integral from
!    XTAB(1) to XTAB(I) of S(X).
!
!    Workspace, real ( kind = DP ) WORK(NTAB).
!
!    Output, real ( kind = DP ) RESULT, the estimated value of the integral.
!
  implicit none
...
!
!    Input, real ( kind = DP ) FTAB(NTAB), contains the tabulated function
...
!    NTAB.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the
!    integral from XTAB(IA) to XTAB(IB) of the function.
!
!    Output, real ( kind = DP ) ERROR, an estimate of the error in
!    integration.
!
  implicit none
...
!
!    Input, real ( kind = DP ) FTAB(NTAB), contains the value of the function
!    at A, A+H, A+2*H, ... , B-H, B, where H = (B-A)/(NTAB-1).
!
!    Input, real ( kind = DP ) A, B, the limits of integration.
!
!    Input, real ( kind = DP ) T, the multiplier of the X argument of the cosine.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
  implicit none
...
!    Input, real ( kind = DP ) A, B, the limits of integration.
!
!    Input, real ( kind = DP ) T, multiplier of the X argument of the sine.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
  implicit none
...
!
!    Output, real ( kind = DP ) GAMMA, the value of the function.  The program
...
!
  ...
  real ( kind = DP ), parameter :: SQRTPI = 0.9189385332046727417803297D+00
  real ( kind = DP ) sum1
  real ( kind = DP ) x
  real ( kind = DP ), parameter :: XBIG = 35.040D+00
  real ( kind = DP ) xden
  real ( kind = DP ), parameter :: XMININ = 1.18D-38
  real ( kind = DP ) xnum
  real ( kind = DP ) y
  real ( kind = DP ) y1
  real ( kind = DP ) ysq
...
!  Parameters:
!
!    Input, real ( kind = DP ), external FUNC, name of external function to
...
!
!    Output, real ( kind = DP ) RESULT, the computed value of the integral.
!
...
!
  ...
  real ( kind = DP ) vl(30)
  real ( kind = DP ) vr
  real ( kind = DP ), save :: w1 = 3.62683783378361983D-01
  real ( kind = DP ), save :: w2 = 3.13706645877887287D-01
  real ( kind = DP ), save :: w3 = 2.22381034453374471D-01
  real ( kind = DP ), save :: w4 = 1.01228536290376259D-01
  real ( kind = DP ) x
  real ( kind = DP ), save :: x1 = 1.83434642495649805D-01
  real ( kind = DP ), save :: x2 = 5.25532409916328986D-01
  real ( kind = DP ), save :: x3 = 7.96666477413626740D-01
  real ( kind = DP ), save :: x4 = 9.60289856497536232D-01
!
...
!
  ...
  anib = log10 ( 2.0D+00 ) * real ( k, kind = DP ) / 0.30102000D+00
  nbits = int ( anib )
...
!         -1 < ALPHA
!
!    Input, integer ( kind = 4 ) NORDER, the number of points used for 
!    the quadrature rule.
!
!    Input, real ( kind = DP ) ALPHA, is only required for Gauss-Jacobi and
!    Gauss-Laguerre quadrature.  Its value is ignored in other cases.
!
!    Input, real ( kind = DP ) BETA, is only required for Gauss-Jacobi
...
!
!    Input, real ( kind = DP ) ENDPTS(2).
!    If KPTS is 1 or 2, ENDPTS contains the locations of the
!    endpoints to be fixed.
!
!    Workspace, real ( kind = DP ) B(NORDER).
!
!    Output, real ( kind = DP ) XTAB(NORDER), the nodes for the quadrature rule.
!
!    Output, real ( kind = DP ) WEIGHT(NORDER), the weights for the 
!    quadrature rule.
!
  implicit none
...
!
!    Input, real ( kind = DP ) DELT, the spacing between the X values of the
!    data.  The actual X values are not needed!
!
!    Input, real ( kind = DP ) Y(NTAB), the Y values of the data.
!
!    Work array, real ( kind = DP ) WORK(2*(NTAB-1)).  The actual minimum amount
!    of workspace required is two times the number of integer
!    divisors of NTAB-1.
!
!    Output, real ( kind = DP ) RESULT, the approximation to the integral.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = DP ), external FUNC.
...
!    Input, real ( kind = DP ) B, the upper limit of integration.
!
!    Input, real ( kind = DP ) EPSIN, the requested relative error tolerance.
!
!    Output, real ( kind = DP ) EPSOUT, an estimate of the error in the
!    integration.
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
...
!  Parameters:
!
!    Input, real ( kind = DP ), external FUNC, name of external function to be
...
!    and in ascending order.
!
!    Input, real ( kind = DP ) FTAB(NTAB), the function values, 
!    FTAB(I) = F(XTAB(I)).
!
!    Input, real ( kind = DP ) A, the lower limit of integration.  A should
!    be, but need not be, near one endpoint of the interval
...
!    which evaluates the function at the point X.
!
!    Input, real ( kind = DP ) A, lower limit of integral.
!
!    Input, real ( kind = DP ) B, upper limit of integral.
!
!    Input, real ( kind = DP ) ERR, is a requested error tolerance.
!    Normally pick a value, 0 < ERR < 1.0D-03.
!
!    Output, real ( kind = DP ) RESULT, computed value of the integral.
...
!
  nbits = int ( log10 ( 2.0D+00 ) &
    * real ( digits ( result ), kind = DP ) / 0.30102000 )
...
3 continue
  ...
  do i = 1, 11, 2
    f(i) = func ( a + real ( i - 1, kind = DP ) * hh(1) )
  end do
...
5 continue
  ...
  do i = 2, 12, 2
    f(i) = func ( aa(l) + real ( i - 1, kind = DP ) * hh(l) )
  end do
...
!  Parameters:
!
!    Input, real ( kind = DP ), external FUNC, the name of the function to 
...
!    Input, real ( kind = DP ) RELERR, the relative error tolerance.
!
!    Input, integer ( kind = 4 ) NLEAST, the least number of times the 
!    integration is to be carried out before the convergence test is made.
!    A value 3 <= NLEAST <= 15 is suggested.
!
!    Input, integer ( kind = 4 ) NMOST, the most number of times the
!    integration is to be carried out.
!
!    Workspace, real ( kind = DP ) WORK(NMOST+1).
!
!    Output, real ( kind = DP ) RESULT, the approximate value of the integral.
!
  implicit none
  ...
  do i = 1, nmost
    ...
    sumabs = 0.0D+00
    do j = 1, nx
      fcnxi = func ( a + h * real ( 2 * j - 1, kind = DP ) )
...
!
!    Input, real ( kind = DP ) ALO, AHI, the low and high values.
!
!    Output, real ( kind = DP ) A(N), N evenly spaced values.
...
!    Input, real ( kind = DP ) B, upper limit of integration.
!
!    Input, real ( kind = DP ) EPSIN, requested relative error tolerance.
!
!    Output, real ( kind = DP ) EPSOUT, estimated achieved relative error.
!
...
!
  ...
  real ( kind = DP ) a
  real ( kind = DP ) acof(11)
  real ( kind = DP ) alf
  real ( kind = DP ) alfnj
  real ( kind = DP ) alfno
  real ( kind = DP ) ar
  real ( kind = DP ) b
  real ( kind = DP ) bcof(nupper+1)
  real ( kind = DP ) bet
  real ( kind = DP ) betnj
  real ( kind = DP ) betno
  real ( kind = DP ) const1
  ...
  real ( kind = DP ) epsout
  real ( kind = DP ) error
  real ( kind = DP ), parameter :: fac1 = 0.411233516712057D+00
  real ( kind = DP ), parameter :: fac2 = 0.822467033441132D+00
  real ( kind = DP ) factor
  real ( kind = DP ), external :: func
  real ( kind = DP ) gamman
  real ( kind = DP ) hnstep
...
!    Input, real ( kind = DP ) B, the upper limit of integration.
!
!    Input, real ( kind = DP ) EPS, the requested error tolerance.
!
!    Output, real ( kind = DP ) RESULT, the approximation to the integral.
!
  implicit none
  ...
  real ( kind = DP ) b
  real ( kind = DP ) da
  real ( kind = DP ) dx(maxlev)
  real ( kind = DP ) ep
  real ( kind = DP ) eps
  real ( kind = DP ) epsp(maxlev)
  real ( kind = DP ) est
  real ( kind = DP ) est1
  real ( kind = DP ) est2(maxlev)
  real ( kind = DP ) est3(maxlev)
  real ( kind = DP ) f1
  real ( kind = DP ) f2(maxlev)
  real ( kind = DP ) f3(maxlev)
  real ( kind = DP ) f4(maxlev)
  real ( kind = DP ) fa
  real ( kind = DP ) fb
  real ( kind = DP ) fbp(maxlev)
  real ( kind = DP ) fm
  real ( kind = DP ) fmp(maxlev)
  real ( kind = DP ), external :: func
  integer ( kind = 4 ) l
  integer ( kind = 4 ) lvl
  integer ( kind = 4 ) nrtr(maxlev)
  real ( kind = DP ) pval(maxlev,3)
  real ( kind = DP ) result
  real ( kind = DP ) sum1
  real ( kind = DP ) sx
  real ( kind = DP ) x2(maxlev)
  real ( kind = DP ) x3(maxlev)
...
!
!    Input, real ( kind = DP ) X(NTAB), contains the X values of the data,
!    in order.
!
!    Input, real ( kind = DP ) Y(NTAB), contains the Y values of the data.
!
!    Output, real ( kind = DP ) RESULT.
...
!    NTAB must be at least 3.
!
!    Input, real ( kind = DP ) H, specifies the increment between the
!    X values.  Note that the actual X values are not needed,
!    just the constant spacing!
!
!    Input, real ( kind = DP ) Y(NTAB), the data.
!
...
!  Parameters:
!
!    Input, real ( kind = DP ) SHIFT, the value of the factor that multiplies
...
!
!    Input, real ( kind = DP ) H, is the spacing between the points at which
!    the data was evaluated.
!
!    Input, real ( kind = DP ) FTAB(NTAB), contains the tabulated data values.
!
!    Output, real ( kind = DP ) RESULT, is the approximation to the integral.
!
  implicit none
